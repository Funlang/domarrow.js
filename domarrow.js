window.DOMLineArrow = (panel) => {
    ...
    function intersectionPoint(x1, y1, x2, y2, minX, minY, maxX, maxY) {
        ...
        if (
            ...
            (x1 >= maxX && x2 >= maxX) ||
            (y1 >= maxY && y2 >= maxY) ||
            (inside(minX, minY, maxX, maxY, x1, y1) && inside(minX, minY, maxX, maxY, x2, y2))
        )
            return null;
    ...
    function adjustLine(from, to, line, trafo) {
        ...
        reverse ? line.classList.add('reverse') : line.classList.remove('reverse');
    }
    ...
    function repaintConnection(connectionElement) {
        ...
        var needTail = connectionElement.hasAttribute("tail") && connectionElement.getAttribute("tail") != "0";
        var needHead = connectionElement.hasAttribute("head") && connectionElement.getAttribute("head") != "0";
        ...
        if (textE && textE.innerHTML != textMessage) {
            textE.innerHTML = textMessage;
        }
    ...
    function repaintWithoutObserve(tag) {
        tag.ob_refcount = tag.ob_refcount || 0;
        tag.ob_refcount -= 1;
        connectionObserver.observe(tag, { attributeFilter: [] });
        repaintConnection(tag);
        tab.ob_refcount += 1;
        tab.ob_refcount > 0 && connectionObserver.observe(tag, { attributes: true, childList: true, subtree: true });
    }
